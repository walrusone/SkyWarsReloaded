...
public class ParticleEffectOption extends PlayerOption {

	private static ArrayList<PlayerOption> playerOptions = new ArrayList<>();
    private List<ParticleEffect> effects;
    
    private ParticleEffectOption(String key, List<ParticleEffect> effects, String name, Material material, int level, int cost, int position, int page, int menuSize) {
        this.key = key;
    	this.level = level;
    	this.cost = cost;
 	        ...
 	        if (storage.getConfigurationSection("effects") != null) {
 	        	for (String key: storage.getConfigurationSection("effects").getKeys(false)) {
                	...
                	int menuSize = storage.getInt("menuSize");
 	            	...
 	            	if (mat != null) {
 	            		playerOptions.add(new ParticleEffectOption(key, effects, name, mat, level, cost, position, page, menuSize));
 	                }
 	        	}
 	        }
 	    }
	...
	public String getPermission() {
		return ("sw.pareffect." + key);
	}

	@Override
	public String getMenuName() {
		return "menu.useeffect-menu-title";
	}

	@Override
	public String getPurchaseMessage() {
		return new Messaging.MessageFormatter().setVariable("cost", "" + cost)
				.setVariable("item", name).format("menu.purchase-effect");
	}
