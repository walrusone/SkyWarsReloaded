...
public class WinSoundOption extends PlayerOption {
    ...
    public WinSoundOption(String key, String sound, String displayname, int level, int cost, float volume, float pitch, Material material, boolean customSound, int position, int page, int menuSize) {
        this.level = level;
        this.cost = cost;
        this.key = key;
        this.sound = sound;
                 	...
                 	if (mat != null) {
                 		if (!isCustom) {
                 			try {
                 				...
                 				if (s != null) {
                     				playerOptions.add(new WinSoundOption(key, sound, name, level, cost, volume, pitch, mat, isCustom, position, page, menuSize));
                 				}
