...
public class OptionSelectionMenu {
    ...
    public OptionSelectionMenu(final Player player, PlayerOptions p, boolean commandOpen) {
        ...
        menuName = new Messaging.MessageFormatter().format(availableItems.get(0).getMenuName());
        menuSize = availableItems.get(0).getMenuSize();
        ...
        for (int iii = 0; iii < availableItems.size(); iii ++) { 
        	PlayerOption option = availableItems.get(iii);
        ...
        SkyWarsReloaded.getIC().create(player, invs, new com.walrusone.skywarsreloaded.menus.IconMenu.OptionClickEventHandler() {
			@Override
