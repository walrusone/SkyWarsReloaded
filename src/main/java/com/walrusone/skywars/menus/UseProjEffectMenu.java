...
public class UseProjEffectMenu {
    ...
    public UseProjEffectMenu(final GamePlayer gamePlayer) {
		...
		String particleItem = SkyWarsReloaded.get().getConfig().getString("gameItems.trailEffectMenuItem");
        ...
        for (int iii = 0; iii < availableItems.size(); iii ++) {
            ...
            if (hasEffectPermission(gamePlayer, effect)) {
            	...
            	loreList.add(new Messaging.MessageFormatter().format("menu.useprojeffect-no-use"));
            }
        ...
        loreList2.add(new Messaging.MessageFormatter().format("menu.useprojeffect-seteffect"));
        SkyWarsReloaded.getIC().setOption(
	                    gamePlayer.getP(),
	                    36,
